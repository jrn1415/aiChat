...
describe('Integration Tests - Chat Application', () => {
    ...
    test('สถานการณ์จริง: การแจ้งเตือนเข้า-ออกของผู้ใช้', (done) => {
        ...
        const checkNotifications = () => {
            // คาดหวังการแจ้งเตือน: user1 เข้า, user2 เข้า, user1 ออก
            if (notifications.length === 3) {
                expect(notifications[0]).toContain('เข้าร่วม');
                expect(notifications[1]).toContain('เข้าร่วม');
                expect(notifications[2]).toContain('ออกจาก');
                done();
            }
        };
        
        const setupNotificationListener = (client) => {
            client.on('user joined', (data) => {
                notifications.push(`${data.username} เข้าร่วม`);
                checkNotifications();
            });
            
            client.on('user left', (data) => {
                notifications.push(`${data.username} ออกจาก`);
                checkNotifications();
            });
        };
        ...
        const checkReady = () => {
            ...
            if (connectedCount === 2) {
                // ตั้งชื่อผู้ใช้
                helper1.setUsername('User1');
                helper2.setUsername('User2');
                
                // ตั้งค่า listeners
                setupNotificationListener(client1);
                setupNotificationListener(client2);
    ...
    test('สถานการณ์จริง: การแสดงสถานะพิมพ์', (done) => {
        ...
        const checkReady = () => {
            ...
            if (connectedCount === 2) {
                // ตั้งชื่อผู้ใช้
                helper1.setUsername('TyperUser');
                helper2.setUsername('WatcherUser');
                ...
                client2.on('typing', (data) => {
                    expect(data.username).toBe('TyperUser');
                    typingReceived = true;
                    checkComplete();
                });
                ...
                setTimeout(() => {
                    ...
                    // หยุดพิมพ์หลังจากนั้น
                    setTimeout(() => {
                        client1.emit('stop typing', helper1.createTypingData());
                    }, 200);
                }, 200);
            }
        };
    ...
    test('การทดสอบความเสถียร: การเชื่อมต่อผู้ใช้หลายคน', (done) => {
        const numberOfUsers = 3; // ลดจำนวนเพื่อให้ง่ายต่อการ debug
